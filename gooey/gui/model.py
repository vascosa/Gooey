...
class MyWidget(object):
    ...
    def __init__(self, type, title, help, default, nargs, commands, choices, widgets):
        self.type = type
        self.title = title
        self.help = help
    ...
    def value(self):
        ...
        if self.type == 'RadioGroup':
            try:
                index, value = self.widget_instance.get_value()
                if index != None:
                    self.widgets[index]._value = value
        ...
        if self.type == 'Textarea':
            if self.commands and self._value:
                return '{} {}'.format(self.commands[0],
                                      quote(self._value.encode('unicode_escape')))
            else:
                return quote(
                    self._value.encode('unicode_escape')) if self._value else ''
        ...
        if self.type == 'Listbox':
            if self.commands and self._value:
                return u'{} {}'.format(self.commands[0],
                                       ' '.join(map(quote, self._value)))
            else:
...
class MyModel(object):
    '''
    What in the hell was I smoking 4 years ago that lead to
    ...
    def __init__(self, build_spec):
        ...
        self.disable_progress_bar_animation = self.build_spec[
    ...
    def is_required_section_complete(self):
        ...
        for arg in self.required_args:
            if arg.type == 'RadioGroup':
                active_widget = arg.widget_instance.active_widget()
                if active_widget == None:
                    error_found = True
                    continue
                else:
                    if not active_widget.get_value():
                        error_found = True
                continue
                # for w in :
                #     if
                #     if w.get_value() == [None, None]:
                #         error_found = True
                # continue
    ...
    def group_arguments(self, widget_list):
        ...
        return list(map(self.to_object, required_args)), \
               list(map(self.to_object, optional_args))


    @staticmethod
    def partition(collection, condition):
        return list(filter(condition, collection)), list(
            filter(lambda x: not condition(x), collection))
    ...
    def to_object(self, data):
        ...
        return MyWidget(
            ...
            list(map(self.to_object, details['widgets'])) if details.get('widgets') else []
        )
